...
//   FaBell, FaEnvelope, FaCalendarAlt, FaTimes, FaCheck,
//   FaCheckCircle, FaTimesCircle, FaClock, FaExclamationCircle,
//   FaChevronDown, FaReply, FaArrowLeft, FaVideo, FaExternalLinkAlt
// } from "react-icons/fa";
// import RequestRescheduleModal from "../components/RequestRescheduleModal";
...
//   // Auto-select notification from navigation state
//   useEffect(() => {
//     if (location.state?.notificationId) {
//       // Wait for notifications to load, then find and select it
...
//         // Mark consultation notification as read - implement if needed
...
//     if (filter === 'feedback') return n.type === 'feedback';
//     if (filter === 'appointments') return n.type === 'appointment';
//     if (filter === 'consultations') return n.type === 'consultation';
//     return true;
//   });

//   // ---------------- DETAIL VIEWS ----------------

//   const renderConsultationDetail = (notification) => (
//     <div className="bg-white rounded-xl shadow-lg p-8">
...
//           <FaVideo className="text-indigo-600" /> Consultation Approved
//         </h2>
//         <p className="text-sm text-indigo-700">
//           Service: {notification.serviceName || 'Video Consultation'}
//         </p>
//       </div>
...
//           <p className="text-lg font-bold text-gray-800">
//             {notification.scheduledDate ? new Date(notification.scheduledDate).toLocaleDateString('en-US', {
//               weekday: 'long', year: 'numeric', month: 'long', day: 'numeric'
//             }) : 'N/A'}
//           </p>
//         </div>
//         <div className="bg-gray-50 p-4 rounded-lg">
//           <p className="text-sm text-gray-600 font-semibold">Scheduled Time</p>
//           <p className="text-lg font-bold text-gray-800">{notification.scheduledTime || 'N/A'}</p>
//         </div>
//       </div>
...
//             <input
//               type="text"
//               value={notification.meetingLink}
//               readOnly
//               className="flex-1 p-2 border border-gray-300 rounded-lg bg-white text-sm text-gray-700"
//             />
//             <button
//               onClick={() => {
...
//             className="flex-1 bg-green-600 text-white px-4 py-3 rounded-lg hover:bg-green-700 transition font-semibold text-center flex items-center justify-center gap-2"
//           >
//             <FaVideo /> Join Meeting <FaExternalLinkAlt size={12} />
//           </a>
//         )}
//         <button
//           onClick={() => {
...
//   const renderFeedbackDetail = (notification) => (
//     <div className="bg-white rounded-xl shadow-lg p-8">
//       <div className="bg-blue-50 p-6 rounded-lg border border-blue-200 mb-6">
//         <h2 className="text-2xl font-bold text-blue-900 mb-2 flex items-center gap-2">
//           <FaEnvelope className="text-blue-600" /> Admin Feedback
//         </h2>
//         <p className="text-sm text-blue-700">Complaint #: {notification.complaintNumber}</p>
//       </div>

//       <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 mb-6">
//         <h3 className="font-semibold text-gray-800 mb-3">Message from Admin:</h3>
//         <p className="text-gray-700 whitespace-pre-wrap text-lg">{notification.message}</p>
//       </div>
...
//   const renderAppointmentDetail = (notification) => (
//     <div className="bg-white rounded-xl shadow-lg p-8">
//       <div className="bg-green-50 p-6 rounded-lg border border-green-200 mb-6">
//         <h2 className="text-2xl font-bold text-green-900 mb-2 flex items-center gap-2">
//           <FaCalendarAlt className="text-green-600" /> Appointment
//         </h2>
//         <p className="text-sm text-green-700">Complaint #: {notification.complaintNumber}</p>
//       </div>
...
//         <div className="bg-gray-50 p-4 rounded-lg">
//           <p className="text-sm text-gray-600 font-semibold">Time</p>
//           <p className="text-lg font-bold text-gray-800">{notification.time}</p>
//         </div>
//         <div className="bg-gray-50 p-4 rounded-lg col-span-2">
//           <p className="text-sm text-gray-600 font-semibold">Location</p>
//           <p className="text-lg font-bold text-gray-800">{notification.location}</p>
//         </div>
//       </div>
...
//         <p className="text-sm text-gray-600 font-semibold mb-2">Your Response:</p>
//         <span className={`inline-block px-4 py-2 rounded-full font-semibold text-sm ${
//           notification.userResponseStatus === 'Accepted' ? 'bg-green-100 text-green-800' :
//           notification.userResponseStatus === 'Declined' ? 'bg-red-100 text-red-800' :
//           notification.userResponseStatus === 'Requested Reschedule' ? 'bg-yellow-100 text-yellow-800' :
//           'bg-gray-100 text-gray-800'
//         }`}>
//           {notification.userResponseStatus || 'Pending'}
//         </span>
//       </div>

//       {/* Reschedule button and requests */}
//       {notification.type === 'appointment' && notification.userResponseStatus !== 'Declined' && notification.userResponseStatus !== 'Requested Reschedule' && (
//         <div className="mb-6 flex gap-2">
//           <button
//             onClick={() => {
//               const appointmentData = {
//                 _id: notification.appointmentId,
//                 appointmentDate: notification.date,
//                 appointmentTime: notification.time,
...
//       {notification.rescheduleRequests && notification.rescheduleRequests.length > 0 && (
//         <div className="bg-purple-50 p-6 rounded-lg border border-purple-200">
//           <h3 className="font-bold text-purple-900 mb-4 flex items-center gap-2">
//             <FaClock /> Reschedule Requests
//           </h3>
//           {notification.rescheduleRequests.map((req, idx) => (
//             <div key={idx} className="bg-white p-4 rounded-lg border border-purple-200 mb-3">
//               {/* request details */}
//               <div className="grid grid-cols-2 gap-4 mb-3">
//                 <div>
//                   <p className="text-xs text-gray-600 font-semibold">Proposed Date</p>
//                   <p className="font-semibold text-purple-900">{new Date(req.proposedDate).toLocaleDateString()}</p>
//                 </div>
//                 <div>
//                   <p className="text-xs text-gray-600 font-semibold">Proposed Time</p>
//                   <p className="font-semibold text-purple-900">{req.proposedTime}</p>
...
//                 </div>
//               </div>
//               <p className="text-sm text-gray-600 mb-3"><strong>Reason:</strong> {req.reason}</p>
//               <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${
//                 req.status === 'Pending' ? 'bg-yellow-100 text-yellow-800' :
//                 req.status === 'Approved' ? 'bg-green-100 text-green-800' :
//                 'bg-red-100 text-red-800'
...
//                   <button
//                     onClick={() => handleApproveReschedule(notification.appointmentId, req._id)}
...
//                   </button>
//                   <button
//                     onClick={() => handleRejectReschedule(notification.appointmentId, req._id, 'Your reschedule request has been rejected.')}
...
//               )}
//               {req.status !== 'Pending' && (
//                 <p className="text-sm text-gray-600 italic mt-2">{req.adminResponse}</p>
//               )}
//             </div>
//           ))}
//         </div>
...
//     if (selectedNotification.type === 'consultation') {
//       detailContent = renderConsultationDetail(selectedNotification);
//     } else if (selectedNotification.type === 'feedback') {
//       detailContent = renderFeedbackDetail(selectedNotification);
//     } else {
//       detailContent = renderAppointmentDetail(selectedNotification);
//     }
...
//             Messages & Notifications
//           </h1>
//           <p className="text-gray-600">View admin feedback, appointments, and video consultation updates</p>
//         </div>
...
//             <FaBell className="text-6xl text-gray-300 mx-auto mb-4" />
//             <p className="text-gray-600 font-semibold">No notifications yet</p>
//             <p className="text-gray-400 text-sm">You'll see updates from admin here</p>
//           </div>
//         ) : (
//           <div className="space-y-4">
//             {filteredNotifications.map(notification => (
...
//                 <div className="flex items-start gap-4">
//                   <div className={`p-3 rounded-full flex-shrink-0 ${
//                     notification.type === 'feedback' ? 'bg-blue-100' :
//                     notification.type === 'consultation' ? 'bg-indigo-100' :
//                     'bg-green-100'
//                   }`}>
//                     {notification.type === 'feedback' ? (
...
//                       <div>
//                         <h3 className="font-bold text-gray-800 flex items-center gap-2">
//                           {notification.type === 'feedback' && 'Feedback from Admin'}
//                           {notification.type === 'consultation' && 'Consultation Approved'}
//                           {notification.type === 'appointment' && 'Appointment Scheduled'}
//                           {!notification.isRead && (
//                             <span className="inline-block w-3 h-3 bg-blue-600 rounded-full"></span>
//                           )}
//                         </h3>
//                         {notification.type === 'consultation' && (
//                           <p className="text-sm text-gray-500">{notification.serviceName || 'Video Consultation'}</p>
//                         )}
//                         {notification.type !== 'consultation' && (
//                           <p className="text-sm text-gray-500">
//                             Complaint #{notification.complaintNumber || 'N/A'}
//                           </p>
//                         )}
//                       </div>
//                       <span className="text-xs text-gray-500">
...
export default function UserNotifications() {
  ...
  useEffect(() => {
    if (location.state?.notificationId) {
      ...
      const found = notifications.find(n => n._id === id);
      if (found) {
        console.log("Auto-selected notification:", found); // Debug
        setSelectedNotification(found);
        navigate(location.pathname, { replace: true, state: {} });
      }
    }
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      // ✅ Use the correct endpoint: /api/notifications (not /api/users/notifications)
      const response = await axios.get(
  ...
  const markAsRead = async (notification) => {
    try {
      ...
      if (notification.type === 'feedback') {
        ...
        // For consultation, we just mark it read in the backend (if you have an endpoint)
        // If not, just update local state
        await axios.put(
