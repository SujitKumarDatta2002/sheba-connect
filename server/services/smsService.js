...
async function sendViaWhatsApp(phone, message) {
  ...
  try {
    ...
    return { success: true, sid: msg.sid, channel: 'whatsapp', status: msg.status };
  } catch (twilioError) {
    console.error(`[WhatsApp] ❌ Failed to send to ${to}:`, twilioError.message);
    console.error(`[WhatsApp] Error code: ${twilioError.code}`);
    console.error(`[WhatsApp] More info: ${twilioError.moreInfo}`);
    throw new Error(`WhatsApp send failed: ${twilioError.message} (Code: ${twilioError.code})`);
  }
}
