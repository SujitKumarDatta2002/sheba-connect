...
// const { buildComplaintImageRecord, isSupportedComplaintImage } = require('../utils/complaintImage');
...
// // CREATE a new complaint
// router.post('/create', authMiddleware, complaintUpload.single('image'), async (req, res) => {
//   try {
//     console.log("Received complaint data:", req.body);
...
//     if (req.file && !isSupportedComplaintImage(req.file.mimetype)) {
//       return res.status(400).json({ message: 'Only image files are allowed for complaints' });
//     }
...
//     if (req.file) {
//       const relativePath = `/uploads/complaints/${path.basename(req.file.path)}`;
//       complaintData.attachments.push(buildComplaintImageRecord(req.file, relativePath));
//     }
...
// ── Helper: get GridFS bucket for complaint attachments ──
function getGridFSBucket() {
  const conn = mongoose.connection;
  if (!conn || !conn.db) {
    throw new Error('MongoDB connection not established');
  }
  return new GridFSBucket(conn.db, { bucketName: 'complaintAttachments' });
}
...
// ── CREATE complaint with image attachment (GridFS) ──────────────────────
router.post('/create', authMiddleware, complaintUpload.single('image'), async (req, res) => {
  try {
    console.log("Received complaint data:", req.body);
    ...
    // ── If an image was uploaded, store it in GridFS ──
    if (req.file) {
      ...
      const attachment = {
        ...
        path: `/api/complaints/attachment/${fileId}`,  // URL to retrieve it
...
// ── GET attachment by GridFS file ID ─────────────────────────────────────
router.get('/attachment/:fileId', authMiddleware, async (req, res) => {
  try {
...
// ── Other routes (status update, edit, delete, feedback, etc.) ────────────
// ... (keep your existing routes for status, edit, delete, feedback, respond)
// They remain unchanged because they don't touch attachments.
