...
const getUploadedDocumentTypes = async (userId, requiredDocs = []) => {
...
const reviewApplication = async (req, res) => {
  try {
    ...
    const service = await Service.findById(application.serviceId).select("requiredDocuments");
    const requiredDocs = service?.requiredDocuments || [];
    const uploadedDocs = await getUploadedDocumentTypes(application.userId, requiredDocs);
    const uploadedTypeSet = new Set(uploadedDocs.map((doc) => doc.documentType));

    const normalizedRequestedFields = requestedFields.filter((field) => !uploadedTypeSet.has(field));
    if (normalizedRequestedFields.length === 0) {
      return res.status(400).json({
        message: "All requested fields are already uploaded by this user. Please select missing documents only."
      });
    }

    application.status = 'AWAITING_USER_DOCS';
    application.requestedFields = normalizedRequestedFields;
  ...
  try {
    ...
    const enriched = await Promise.all(
      applications.map(async (application) => {
        const requiredDocs = application.serviceId?.requiredDocuments || [];
        const uploadedDocs = await getUploadedDocumentTypes(application.userId?._id || application.userId, requiredDocs);
